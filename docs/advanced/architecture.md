---
title: Architecture
description: Internal architecture of the Vue Three Fiber renderer.
---

# Architecture

This page describes the internal structure of Vue Three Fiber. It is intended for contributors and advanced users who want to understand how the renderer works under the hood. All source references point to files in `packages/fiber/src/`.
...
Vue Three Fiber uses Vue's `createRenderer()` API to build a custom renderer whose host nodes are Three.js objects. The renderer is defined by a `nodeOps` object that implements Vue's `RendererOptions<Instance, Instance>` interface.
