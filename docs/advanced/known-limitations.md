...
description: Runtime behaviors and constraints to be aware of when using Vue Three Fiber
---

## Refs are proxy-backed

Template refs in Vue Three Fiber are **proxy-backed handles**, not raw `THREE.Object3D` instances. Property access, method calls, and `instanceof` all work transparently through the proxy.
...
Vue Three Fiber's pointer events are raycaster-based, not DOM-based. Some differences from standard DOM pointer events:
...
Vue Three Fiber uses Vue's `createRenderer` API to render into Three.js instead of the DOM. This means:
