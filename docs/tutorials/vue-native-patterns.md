...
description: Patterns unique to Vue Three Fiber that have no React Three Fiber equivalent.
---

# Vue-Native Patterns

Vue Three Fiber is a fiber-style Three.js renderer for Vue, compatible with React Three Fiber's API where practical and Vue-native where stronger. This guide covers patterns that take advantage of Vue's reactivity system, composition API, and component model in ways that are not available in the React ecosystem.
...
Vue Three Fiber provides three composables for coordinating Vue reactivity with the Three.js render loop. Each serves a different use case.
...
Vue Three Fiber includes a plugin system that lets ecosystem packages hook into the renderer lifecycle without tight coupling to Canvas internals.
