...
It would be tempting to just update the reactive state of our component and let it change the `mesh` via props, but going through reactive state isn't ideal when dealing with continuous updates, commonly known as transient updates.
Instead, we want to **directly mutate our mesh each frame**. First, we'll have to get a `reference` to it, via Vue's `ref()`:
...
Now that you understand the basic technique for animating in Fiber, [learn how events work](/tutorials/events-and-interaction)!
