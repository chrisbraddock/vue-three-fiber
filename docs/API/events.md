...
Because events go to all intersected objects, capturing the pointer also works differently. In the DOM, the capturing object **replaces** the hit test, but in vue-threejs, the capturing object is **added** to the hit test result: if the capturing object was not hit, then all of the hit objects (and their ancestors) get the event first, followed by the capturing object and its ancestors. The capturing object can also use `event.stopPropagation()` so that objects that really were hit get pointerout events.
...
onMounted(() => {
  events.value.connect(domNode)
})
```

Or, the `eventSource` shortcut on the canvas (DOM only), which accepts dom-nodes and Vue refs to dom-nodes.
...
<template>
  <div ref="target">
    <Canvas :eventSource="target"> </Canvas>
  </div>
</template>
```
...
```vue
<template>
  <Canvas eventPrefix="client"> </Canvas>
</template>
```
...
```js
function useRaycastOnCameraMove() {
