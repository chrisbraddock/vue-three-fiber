...
export const CuboidCollider = defineComponent({
  ...
  setup(props) {
    ...
    if (!physics) {
      throw new Error('@bluera/vue-threejs-rapier: <CuboidCollider> must be placed inside a <Physics> component')
    }

    const rigidBody = inject(RIGID_BODY_CONTEXT)
    if (!rigidBody) {
      throw new Error('@bluera/vue-threejs-rapier: <CuboidCollider> must be placed inside a <RigidBody> component')
    }
