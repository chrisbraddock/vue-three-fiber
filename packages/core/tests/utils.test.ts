...
function createMockStore(): RootStore {
  let store!: RootStore
  try {
    store = createRoot(document.createElement('canvas')).render(null)
...
describe('getInstanceProps', () => {
  it('should filter internal props without accessing them', () => {
    ...
    const filtered = getInstanceProps(VUE_INTERNAL_PROPS.reduce((acc, key) => ({ ...acc, [key]: { get, set } }), props))
...
describe('prepare', () => {
  it('should create an instance descriptor', () => {
    ...
    expect(object.__v3f).toBe(instance)
  })

  it('should not overwrite descriptors', () => {
    const containerDesc = {}
    const container = { __v3f: containerDesc }

    const instance = prepare(container, store, 'container', {})
    expect(container.__v3f).toBe(containerDesc)
...
describe('applyProps', () => {
  ...
  it('should not throw when applying unknown props', () => {
    ...
    expect((target as unknown as Record<string, unknown>)['foo-bar']).toBe(undefined)
  })
