...
export interface InternalState {
  ...
  lastEvent: MutableRef<DomEvent | null>
  active: boolean
  priority: number
  frames: number
  subscribe: (callback: MutableRef<RenderCallback>, priority: number, store: RootStore) => () => void
}
...
export const context: InjectionKey<RootStore> = /* @__PURE__ */ Symbol('v3f-store')
...
export const createStore = (
  ...
  const rootStore = createZustandStore<RootState>((set, get) => {
    ...
    const rootState: RootState = {
      ...
      internal: {
        ...
        frames: 0,
        priority: 0,
        subscribe: (ref: MutableRef<RenderCallback>, priority: number, store: RootStore) => {
