...
export function createEvents(store: RootStore) {
  ...
  function filterPointerEvents(objects: THREE.Object3D[]) {
    return objects.filter((obj) =>
      ['Move', 'Over', 'Enter', 'Out', 'Leave'].some(
        (name) =>
          (obj as Instance<THREE.Object3D>['object']).__v3f?.handlers[('onPointer' + name) as keyof EventHandlers],
      ),
    )
  }
  ...
  function handleIntersects(
    ...
    if (intersections.length) {
      ...
      for (const hit of intersections) {
        ...
        // If the object is not managed by V3F, it might be parented to an element which is.
        ...
        if (state) {
          ...
          let raycastEvent = {
            ...
            stopPropagation() {
              // https://github.com/pmndrs/vue-three-fiber/issues/596
              ...
              if (
                ...
                if (
                  ...
                  Array.from(internal.hovered.values()).find((i: any) => i.eventObject === hit.eventObject)
                ) {
            ...
            target: { hasPointerCapture, setPointerCapture, releasePointerCapture } as unknown as EventTarget,
            currentTarget: { hasPointerCapture, setPointerCapture, releasePointerCapture } as unknown as EventTarget,
            nativeEvent: event,
  ...
  function cancelPointer(intersections: Intersection[]) {
    ...
    for (const hoveredObj of internal.hovered.values()) {
      ...
      if (
        ...
        const instance = (eventObject as Instance<THREE.Object3D>['object']).__v3f
  ...
  function pointerMissed(event: MouseEvent, objects: THREE.Object3D[]) {
    for (let i = 0; i < objects.length; i++) {
      const instance = (objects[i] as Instance<THREE.Object3D>['object']).__v3f
